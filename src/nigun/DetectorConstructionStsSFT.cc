...
#include "DetectorConstructionStsSFT.hh"
...
DetectorConstructionStsSFT::DetectorConstructionStsSFT()
...
MaterialList* DetectorConstructionStsSFT::DefineMaterial()
{
...
G4VPhysicalVolume* DetectorConstructionStsSFT::Construct()
{
  ...
  logicWorld = new G4LogicalVolume(solidWorld, mList_->Air, "World");
...
void DetectorConstructionStsSFT::MakeField()
{
...
//   fieldManager->SetChordFinder(pChordFinder);
  ...
  fieldManager->GetChordFinder()->SetDeltaChord( 1.0E-2*mm);
...
void DetectorConstructionStsSFT::ConstructQ1(G4VPhysicalVolume *pMother)
{
...
void DetectorConstructionStsSFT::ConstructQ2(G4VPhysicalVolume *pMother)
{
...
void DetectorConstructionStsSFT::ConstructD1(G4VPhysicalVolume *pMother)
{
...
void DetectorConstructionStsSFT::MakePositionDetector(G4VPhysicalVolume *pMother)
{
  ...
  // Material
  G4Material *DCuAreaMater = mList_->Scin;
  G4Material *DCAreaMater = mList_->Ar50Ethane50Gas;
  G4Material *DCLayerMater = mList_->Ar80IsoButane20Gas;
  G4Material *DCuLayerMater = mList_->Scin;
  G4Material *DCuboxMater  = mList_->Scin;
  G4Material *DCboxMater  = mList_->Mylar;

  G4double DCuAreaX = 1000.0;//mm
  G4double DCuAreaY = 1000.0;//mm
  G4double DCuAreaZ = 1.0;//mm
  G4double DCuBoxX = 1000.0;//mm
  G4double DCuBoxY = 1000.0;//mm
  G4double DCuBoxZ = 2.0;//mm

  ///////////////////// BC1(x,v,u) Box=Xplane Area=VUplane
  G4Box *solidBc1BoxX =
    new G4Box( "BC1BoxX", DCuBoxZ/2.0*mm, DCuBoxX/2.0*mm, DCuBoxY/2.0*mm );
  G4Box *solidBc1BoxV =
    new G4Box( "BC1BoxV", DCuAreaZ/2.0*mm, DCuAreaX/2.0*mm, DCuAreaY/2.0*mm );
  G4Box *solidBc1BoxU =
    new G4Box( "BC1BoxU", DCuAreaZ/2.0*mm, DCuAreaX/2.0*mm, DCuAreaY/2.0*mm );
  G4Box *solidBc1Layer =
    new G4Box( "BC1Layer", 0.001/2.0*mm, DCuAreaX/2.0*mm, DCuAreaY/2.0*mm );

  G4LogicalVolume *logBc1BoxX =
    new G4LogicalVolume( solidBc1BoxX,   mList_->HeGas/*DCuboxMater*/,  "BC1BoxX",   0, 0, 0 );
  G4LogicalVolume *logBc1BoxV =
    new G4LogicalVolume( solidBc1BoxV,   mList_->HeGas/*DCuboxMater*/,  "BC1BoxV",   0, 0, 0 );
  G4LogicalVolume *logBc1BoxU =
    new G4LogicalVolume( solidBc1BoxU,  mList_->HeGas/*DCuAreaMater*/,  "BC1BoxU",  0, 0, 0 );
  G4LogicalVolume *logBc1Layer =
    new G4LogicalVolume( solidBc1Layer, mList_->HeGas/*DCuLayerMater*/,  "BC1Layer", 0, 0, 0 );

  G4int lnum_bc1;
  lnum_bc1 = geomMan.GetDetectorId("BC1-v-1");
  ...
  char Bc1Name[3][100] = {"BC1-x-1", "BC1-v-1", "BC1-u-1"};

  G4VPhysicalVolume *physBc1Layer[3];
  for (int i=0; i<3; i++) {
    ...
    if(i==0){
      G4VPhysicalVolume *physBc1X =
	new G4PVPlacement( G4Transform3D(RMBc1, posBc1L1), 
			   "BC1X", logBc1BoxX, pMother, false, 0 );
      physBc1Layer[i] =
	new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, 0.0*mm ),
			   Bc1Name[i], logBc1Layer, physBc1X, false,  101+i );
      G4cout<<posBc1L1<<G4endl;
    }
    if(i==1){
      G4VPhysicalVolume *physBc1V =
	new G4PVPlacement( G4Transform3D(RMBc1, posBc1L1), 
			   "BC1V", logBc1BoxV, pMother, false, 0 );
      physBc1Layer[i] =
	new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, 0.0*mm ),
			   Bc1Name[i], logBc1Layer, physBc1V, false,  101+i );
      G4cout<<posBc1L1<<G4endl;
    }
    if(i==2){
      G4VPhysicalVolume *physBc1U =
	new G4PVPlacement( G4Transform3D(RMBc1, posBc1L1), 
			   "BC1U", logBc1BoxU, pMother, false, 0 );
      physBc1Layer[i] =
	new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, 0.0*mm ),
			   Bc1Name[i], logBc1Layer, physBc1U, false,  101+i );
      G4cout<<posBc1L1<<G4endl;
    }
  ...
  ///////////////////// BC2(x,v,u)
  G4Box *solidBc2BoxX =
    new G4Box( "BC2BoxX", DCuBoxZ/2.0*mm, DCuBoxX/2.0*mm, DCuBoxY/2.0*mm );
  G4Box *solidBc2BoxV =
    new G4Box( "BC2BoxV", DCuBoxZ/2.0*mm, DCuBoxX/2.0*mm, DCuBoxY/2.0*mm );
  G4Box *solidBc2BoxU =
    new G4Box( "BC2BoxU", DCuBoxZ/2.0*mm, DCuBoxX/2.0*mm, DCuBoxY/2.0*mm );
  G4Box *solidBc2Area =
  ...
  G4LogicalVolume *logBc2BoxX =
    new G4LogicalVolume( solidBc2BoxX,   DCuboxMater,  "BC2BoxX",   0, 0, 0 );
  G4LogicalVolume *logBc2BoxV =
    new G4LogicalVolume( solidBc2BoxV,   DCuboxMater,  "BC2BoxV",   0, 0, 0 );
  G4LogicalVolume *logBc2BoxU =
    new G4LogicalVolume( solidBc2BoxU,   DCuboxMater,  "BC2BoxU",   0, 0, 0 );
  G4LogicalVolume *logBc2Area =
    new G4LogicalVolume( solidBc2Area,  DCuAreaMater,  "BC2Area",  0, 0, 0 );
  G4LogicalVolume *logBc2Layer =
    new G4LogicalVolume( solidBc2Layer, DCuLayerMater,  "BC2Layer", 0, 0, 0 );

  G4int lnum_bc2;
  lnum_bc2 = geomMan.GetDetectorId("BC2-v-1");
  ...
  char Bc2Name[3][100] = {"BC2-x-1", "BC2-v-1", "BC2-u-1"};

  G4VPhysicalVolume *physBc2Layer[3];
  for (int i=0; i<3; i++) {
    ...
    if(i==0){
      G4VPhysicalVolume *physBc2X =
	new G4PVPlacement( G4Transform3D(RMBc2, posBc2L1), 
			   "BC2X", logBc2BoxX, pMother, false, 0 );
      physBc2Layer[i] =
	new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, 0.0*mm ),
			   Bc2Name[i], logBc2Layer, physBc2X, false,  104+i );
      G4cout<<posBc2L1<<G4endl;
    }
    if(i==1){
      G4VPhysicalVolume *physBc2V =
	new G4PVPlacement( G4Transform3D(RMBc2, posBc2L1), 
			   "BC2V", logBc2BoxV, pMother, false, 0 );
      physBc2Layer[i] =
	new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, 0.0*mm ),
			   Bc2Name[i], logBc2Layer, physBc2V, false,  104+i );
      G4cout<<posBc2L1<<G4endl;
    }
    if(i==2){
      G4VPhysicalVolume *physBc2U =
	new G4PVPlacement( G4Transform3D(RMBc2, posBc2L1), 
			   "BC2U", logBc2BoxU, pMother, false, 0 );
      physBc2Layer[i] =
	new G4PVPlacement( 0, G4ThreeVector( 0.0*mm, 0.0*mm, 0.0*mm ),
			   Bc2Name[i], logBc2Layer, physBc2U, false,  104+i );
      G4cout<<posBc2L1<<G4endl;
    }

  }

  logBc1BoxX->SetVisAttributes(G4VisAttributes(true,G4Colour(1.0, 0.0, 0.0)));//::Invisible);
  logBc1BoxV->SetVisAttributes(G4VisAttributes(true,G4Colour(1.0, 0.0, 0.0)));//::Invisible);
  logBc1BoxU->SetVisAttributes(G4VisAttributes(true,G4Colour(1.0, 0.0, 0.0)));//::Invisible);
  //  logBc1Area->SetVisAttributes(G4VisAttributes(true,G4Colour(0.0, 1.0, 0.0)));
  logBc1Layer->SetVisAttributes(G4VisAttributes(true,G4Colour(0.0, 0.0, 1.0)));//::Invisible);
  logBc2BoxX->SetVisAttributes(G4VisAttributes(true,G4Colour(1.0, 0.0, 0.0)));//::Invisible);
  logBc2BoxV->SetVisAttributes(G4VisAttributes(true,G4Colour(1.0, 0.0, 0.0)));//::Invisible);
  logBc2BoxU->SetVisAttributes(G4VisAttributes(true,G4Colour(1.0, 0.0, 0.0)));//::Invisible);
  //  logBc2Area->SetVisAttributes(G4VisAttributes(true,G4Colour(0.0, 1.0, 0.0)));
  logBc2Layer->SetVisAttributes(G4VisAttributes(true,G4Colour(0.0, 0.0, 1.0)));//::Invisible);
...
//     posBc3L1.rotateZ(bendAngleD*degree);
    ...
    physBc3Layer[i] =
      new G4PVPlacement( 0, G4ThreeVector( posBc3L1D*mm, 0.0*mm, 0.0*mm ),
			 Bc3Name[i], logBc3Layer, physBc3Area, false,  111+i );
  }
...
//     posBc4L1.rotateZ(bendAngleD*degree);
    ...
    physBc4Layer[i] =
      new G4PVPlacement( 0, G4ThreeVector( posBc4L1D*mm, 0.0*mm, 0.0*mm ),
			 Bc4Name[i], logBc4Layer, physBc4Area, false,  115+i );
  }
...
//     posBc5L1.rotateZ(bendAngleD*degree);
    ...
    physBc5Layer[i] =
      new G4PVPlacement( 0, G4ThreeVector( posBc5L1D*mm, 0.0*mm, 0.0*mm ),
			 Bc5Name[i], logBc5Layer, physBc5Area, false,  121+i );
  }
  ...
  HeBagG[2]=G4ThreeVector(-(100.+(Q1+Q1z/2.)),0,0);
  ...
  for(int i=0;i<7;i++){
	...
	G4cout<<i<<" "<<thick[i]<<" "<<HeBagG[i]<<G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void DetectorConstructionStsSFT::MakeSlits(G4VPhysicalVolume *pMother)
{
...
void DetectorConstructionStsSFT::MakeTOFCounter(G4VPhysicalVolume *pMother)
{
...
void DetectorConstructionStsSFT::MakeAerogelCounter(G4VPhysicalVolume* pMother){
...
void DetectorConstructionStsSFT::MakeWaterCounter(G4VPhysicalVolume* pMother){
...
G4bool DetectorConstructionStsSFT::
