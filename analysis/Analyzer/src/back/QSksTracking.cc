...
int main( int argc, char **argv )
{
  ...
  // Q1
  // -0.005 tesla/mm
  // 2*175mm(H) x 2*125mm(V) x 2*250mm (L)

  //  QMag Q1( "Q1", ThreeVector( 1820.87, -58.31, 0.0 ),
  //	   0.0, 90.0, -130., 175., 125., 250., -0.010 );
  QMag Q1( "Q1", ThreeVector( 1855.07, 35.66, 0.0 ),
	   0.0, 90.0, -130., 175., 125., 250., -0.010 );

  FieldMan::GetInstance().AddElement( &Q1 );
...
const int NBin1HPDcIn[11] = {
  100, 250, 250, 250, 250, 500, 500, 500, 500, 500, 500
};
...
void DefineHistograms( const char *filename )
{
  ...
  for( int i=21; i<=30; ++i ){
    std::ostringstream title1, title2, title3;
    title1 << "HitPos [mm] Layer#" << i;
    HB1( 200+i, title1.str().c_str(), NBin1HPDcIn[i-16], 
	 MinHPDcIn[i-16], MaxHPDcIn[i-16] );
    title2 << "Residual [mm] Layer#" << i;
    HB1( 300+i, title2.str().c_str(), 250, -2.5, 2.5 );
    title3 << "Resd.%Pos [mm] Layer#" << i;
    HB2( 400+i, title3.str().c_str(), NBin2HPDcIn[i-16], 
	 MinHPDcIn[i-16], MaxHPDcIn[i-16], 100, -2.5, 2.5 );

    title1 << " <GoodTrack>";
    HB1( 1200+i, title1.str().c_str(), NBin1HPDcIn[i-16], 
	 MinHPDcIn[i-16], MaxHPDcIn[i-16] );
    title2 << " <GoodTrack>";
    HB1( 1300+i, title2.str().c_str(), 250, -2.5, 2.5 );
    title3 << " <GoodTrack>";
    HB2( 1400+i, title3.str().c_str(), NBin2HPDcIn[i-16], 
	 MinHPDcIn[i-16], MaxHPDcIn[i-16], 100, -2.5, 2.5 );
  }
  for( int i=41; i<=52; ++i ){
    std::ostringstream title1, title2, title3;
...
bool ProcessOneEvent( std::ifstream &In )
{
...
#ifdef Interactive
  {
    ...
    std::cout << "[QSksTracking]: St=" << status
