...
Check_Q2effect( void )
{
  ...
  TFile *f2 = new TFile( "Gene6_Q1_100_Q2_101.root" );
  TFile *f3 = new TFile( "Gene6_Q1_100_Q2_102.root" );
  TFile *f4 = new TFile( "Gene6_Q1_100_Q2_104.root" );
  TFile *f5 = new TFile( "Gene6_Q1_100_Q2_108.root" );
  //TTreeReader reader( "g4s2s", gFile );
  ...
  while( reader.Next() ){
	...
	std::cout<<"VP4x:"<<VP4x[0]
		 ...
		 <<", "<<VP4x[4]<<std::endl;
	for(int i=0; i<4; ++i){
	  hVP4x[i]->Fill(VP4x[0],VP4x[0]-VP4x[i+1]);
	  hVP4y[i]->Fill(VP4y[0],VP4y[0]-VP4y[i+1]);
	}
      }
    }
  }
  ...
  c0->cd(1)->SetGrid(1);
  for(int i=0; i<4; ++i){
    hVP2x[3-i]->SetTitle("VP2 X");
    hVP2x[3-i]->GetXaxis()->SetTitle("X [mm]");
    hVP2x[3-i]->GetYaxis()->SetTitle("#DeltaX [mm]");
    if(i==0)
      hVP2x[3-i]->Draw("scat");
    else
  ...
  c0->cd(2)->SetGrid(1);
  for(int i=0; i<4; ++i){
    hVP2y[3-i]->SetTitle("VP2 Y");
    hVP2y[3-i]->GetXaxis()->SetTitle("Y [mm]");
    hVP2y[3-i]->GetYaxis()->SetTitle("#DeltaY [mm]");
    if(i==0)
      hVP2y[3-i]->Draw("scat");
    else
  ...
  c0->cd(3)->SetGrid(1);
  for(int i=0; i<4; ++i){
    hVP4x[3-i]->SetTitle("VP4 X");
    hVP4x[3-i]->GetXaxis()->SetTitle("X [mm]");
    hVP4x[3-i]->GetYaxis()->SetTitle("#DeltaX [mm]");
    if(i==0)
      hVP4x[3-i]->Draw("scat");
    else
  ...
  c0->cd(4)->SetGrid(1);
  for(int i=0; i<4; ++i){
    hVP4y[3-i]->SetTitle("VP4 Y");
    hVP4y[3-i]->GetXaxis()->SetTitle("Y [mm]");
    hVP4y[3-i]->GetYaxis()->SetTitle("#DeltaY [mm]");
    if(i==0)
      hVP4y[3-i]->Draw("scat");
    else
