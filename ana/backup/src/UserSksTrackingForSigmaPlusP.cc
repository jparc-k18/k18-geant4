...
struct Event{
  ...
  double decayProtonVec[3];
  double decayProtonPos[3];
  double cdistDecayProton;
  double thetaDecayProton;
  double chisqrDecayProtonXY;
  double chisqrDecayProtonZ;
  double decayProtonMomCal;	     
  double decayProtonEkinCal;
  ...
  double fClEdepForProtonDecay[FiberLayerNum+1];
  double fClEdepNormForProtonDecay[FiberLayerNum+1];
  double fClPathLengthForProtonDecay[FiberLayerNum+1];
  ...
  double cClEdepForPi;
  double cClEdepForProtonDecay;
  double cRcEdepForScat;
  double cRcEdepForPi;
  double cRcEdepForProtonDecay;
};
static Event event;
...
bool ProcessOneEvent( std::ifstream &In )
{
  ...
  CylindricalAna CylAnaProtonDecay;
  int lnum;
  In >> lnum;
  ...
  std::cout<< "*********************************************" <<std::endl;
	...
	if (pid == 10 || pid == 11)
	  ...
	  CylAnaProtonDecay.SetFiberDataForScatPart(flayer, segment, edep);
	/*
	...
	*/
	...
	if ((clayer ==1 || clayer == 2) && (pid == 10 || pid == 11))
	  CylAna.SetCrystalDataForScatPart(segment, edep);
	else if ((clayer ==3 || clayer == 4) && (pid == 10 || pid == 11))
	...
	else if ((clayer ==1 || clayer == 2) && pid == 14)
	  CylAnaProtonDecay.SetCrystalDataForScatPart(segment, edep);
	else if ((clayer ==3 || clayer == 4) && pid == 14)
	  CylAnaProtonDecay.SetRcDataForScatPart(segment, edep);
	/*
	  std::cout << "Crystal "
	  << ", Segment = " << segment
	  << ", Edep = " << edep
  ...
  */
  ...
  if (fHitNum >= MinHitForP) {
	...
	if (event.scatTarget == 0 || event.NNscatTarget == 0 || event.PiNscatTarget == 0) // proton
	  {
	    M = ProtonMass;
	    p1 = HypBeamVec.mag();
	    E1 = sqrt(SigmaPlusMass*SigmaPlusMass+p1*p1);
		...
		fiberEdep += event.fClEdepForScat[i];

	    double totE_p = fiberEdep+event.cClEdepForScat;
	...
	else if (event.scatTarget == 1 || event.NNscatTarget == 1 || event.PiNscatTarget == 1) // deuteron
	  {
	    M = DeuteronMass;
	    p1 = HypBeamVec.mag();
	    E1 = sqrt(SigmaPlusMass*SigmaPlusMass+p1*p1);
  ...
  int fHitNumProtonDecay = CylAnaProtonDecay.Evaluate();
  bool flagFitProtonDecayXY, flagFitProtonDecayZ;
  if (fHitNumProtonDecay >= MinHitForP) {
    for (int i=1; i<=FiberLayerNum; i++)
      event.fClEdepForProtonDecay[i] = CylAnaProtonDecay.GetFiberClusterEdep(i);
    event.cClEdepForProtonDecay = CylAnaProtonDecay.GetCrystalClusterEdep();

    flagFitProtonDecayXY = CylAnaProtonDecay.FitXYPlane();
    if (flagFitProtonDecayXY)
      flagFitProtonDecayZ = CylAnaProtonDecay.FitZPlane();

    if (flagFitProtonDecayXY && flagFitProtonDecayZ) {

      ThreeVector Vert5(event.vertex5[0], event.vertex5[1], event.vertex5[2]);
      //ThreeVector HypBeamVec = LvRc.vect();
      ThreeVector ScatPos = CylAnaProtonDecay.GetScatPosition();
      ThreeVector ScatVec = CylAnaProtonDecay.GetScatVector();
      ...
      */
      ...
      double costScat=HypBeamVec*ScatVec/(HypBeamVec.mag()*ScatVec.mag());
      double thetaScat=acos(costScat)*Rad2Deg;
      double chisqrXY = CylAnaProtonDecay.GetChisqrXY();
      double chisqrZ = CylAnaProtonDecay.GetChisqrZ();

      event.vertexDecayProton[0] = VertScat.x();
      event.vertexDecayProton[1] = VertScat.y();
      event.vertexDecayProton[2] = VertScat.z();

      event.decayProtonVec[0] = ScatVec.x();
      event.decayProtonVec[1] = ScatVec.y();
      event.decayProtonVec[2] = ScatVec.z();

      event.decayProtonPos[0] = ScatPos.x();
      event.decayProtonPos[1] = ScatPos.y();
      event.decayProtonPos[2] = ScatPos.z();

      event.cdistDecayProton = cdist;
      event.thetaDecayProton = thetaScat;
      event.chisqrDecayProtonXY = chisqrXY;
      event.chisqrDecayProtonZ = chisqrZ;
      ...
      double M;
      double p1, E1;
      double phi = event.thetaDecayProton;
      double p4, E4, Ekin4;
	...
	event.decayProtonMomCal = p4;
	event.decayProtonEkinCal = Ekin4;
	event.thetaDecayProtonCM = thetaCM;
      }

      for (int i=1; i<=FiberLayerNum; i++) {
	double res = CylAnaProtonDecay.GetResidual(i);
	HF1(650+i, res);
	double edepNorm = CylAnaProtonDecay.GetFiberClusterEdepNorm(i);
	event.fClEdepNormForProtonDecay[i] = edepNorm;
	double pathLength = CylAnaProtonDecay.GetFiberPathLength(i);
	event.fClPathLengthForProtonDecay[i] = pathLength;
      }
    }
  }


  if (event.thetaDecayProton>0 && event.thetaScat>0) {
    double Ekin1=0.; // MeV
    for (int i=1; i<=FiberLayerNum; i++)
      if (event.fClEdepForScat[i]>0.)
    ...
    Ekin1 *= 0.001; // GeV
    double momp1 = sqrt(Ekin1*Ekin1+2.*Ekin1*M1) ;
    ThreeVector momVec1 = CylAna.GetScatVector();
    momVec1 *= momp1/momVec1.mag();
    ThreeVector posVec1 = CylAna.GetScatPosition();

    double Ekin2=0.; // MeV
    for (int i=1; i<=FiberLayerNum; i++)
      if (event.fClEdepForScat[i]>0.)
      Ekin2 += event.fClEdepForProtonDecay[i];
    Ekin2 += event.cClEdepForProtonDecay;
    double M2 = ProtonMass;
    Ekin2 *= 0.001; // GeV
    double momp2 = sqrt(Ekin2*Ekin2+2.*Ekin2*M2) ;
    ThreeVector momVec2 = CylAnaProtonDecay.GetScatVector();
    momVec2 *= momp2/momVec2.mag();
    ThreeVector posVec2 = CylAnaProtonDecay.GetScatPosition();

    double cdistTwoProton;
    ThreeVector VertPPScat = VertexPoint3D( posVec1, posVec2, momVec1, momVec2, cdistTwoProton);

    // Energy Correction 
    momp1 /= 1000.; // GeV/c
    double momp1_cor, ep1_cor;
    CorrElossOut(&momp1_cor, &ep1_cor, momp1, PROTON, momVec1, VertPPScat);
    momVec1 *= momp1_cor/momVec1.mag();

    momp2 /= 1000.; // GeV/c
    double momp2_cor, ep2_cor;
    CorrElossOut(&momp2_cor, &ep2_cor, momp2, PROTON, momVec2, VertPPScat);
    momVec2 *= momp2_cor/momVec2.mag();

    // primary proton
    ThreeVector momVec0 = momVec1 + momVec2;
    double momp0 = momVec0.mag();

    ThreeVector Vert5(event.vertex5[0], event.vertex5[1], event.vertex5[2]);

    double cdistSigmaProtonDecay;

    ThreeVector VertSigmaProtonDecay = VertexPoint3D( Vert5, VertPPScat, HypBeamVec, momVec0, cdistSigmaProtonDecay);
    double costSigmaProtonDecay=HypBeamVec*momVec0/(HypBeamVec.mag()*momVec0.mag());
    ...
    event.momSigmaProtonDecayVec[0] = momVec0.x();
    event.momSigmaProtonDecayVec[1] = momVec0.y();
    event.momSigmaProtonDecayVec[2] = momVec0.z();
...
void InitializeEvent( void )
{
  ...
  event.decayProtonVec[0] = -999.9;
  event.decayProtonVec[1] = -999.9;
  event.decayProtonVec[2] = -999.9;
  
  event.decayProtonPos[0] = -999.9;
  event.decayProtonPos[1] = -999.9;
  event.decayProtonPos[2] = -999.9;
  
  event.cdistDecayProton = -999.9;
  event.thetaDecayProton = -999.9;
  event.chisqrDecayProtonXY = -999.9;
  event.chisqrDecayProtonZ = -999.9;
  event.decayProtonMomCal = -999.9;
  event.decayProtonEkinCal = -999.9;
  ...
  for (int i=0; i<FiberLayerNum+1; i++) {
    event.fClEdepForScat[i] = 0.0;
    event.fClEdepNormForScat[i] = 0.0;
    event.fClPathLengthForScat[i] = -999.0;

    event.fClEdepForPi[i] = 0.0;
    event.fClEdepNormForPi[i] = 0.0;
    event.fClPathLengthForPi[i] = -999.0;

    event.fClEdepForProtonDecay[i] = 0.0;
    event.fClEdepNormForProtonDecay[i] = 0.0;
    event.fClPathLengthForProtonDecay[i] = -999.0;
  }

  event.cClEdepForScat = 0.0;
  event.cClEdepForPi = 0.0;
  event.cClEdepForProtonDecay = 0.0;
  event.cRcEdepForScat = 0.0;
  event.cRcEdepForPi = 0.0;
  event.cRcEdepForProtonDecay = 0.0;
...
void DefineHistograms( const char *filename )
{
  ...
  HB1( 620, "Scattering Vertex X (Hypbeam and Decay Proton)", 500, -100, 100 );
  HB1( 621, "Scattering Vertex Y (Hypbeam and Decay Proton)", 500, -100, 100 );
  HB1( 622, "Scattering Vertex Z (Hypbeam and Decay Proton)", 500, -300, 300 );
  HB1( 623, "Scattering Theta (Hypbeam and Decay Proton)", 360, 0, 180 );
  HB1( 624, "chisqrXY (Decay Proton)", 500, 0., 100. );
  HB1( 625, "chisqrZ (Decay Proton)", 500, 0., 100. );
  HB1( 626, "Closest Distance (Hypbeam and Decay Proton)", 500, 0., 100. );

  for( int i=1; i<=FiberLayerNum; ++i ){
    std::ostringstream title1,title2;
    title1 << "Residual[mm] Layer#" << i;
    HB1( 630+i, title1.str().c_str(), 100, -2.5, 2.5 );
    title1 << "Residual[mm] (decay proton) Layer#" << i;
    HB1( 650+i, title1.str().c_str(), 100, -2.5, 2.5 );
  }  
  ...
  tree->Branch("decayNMomVecCal",   event.decayNMomVecCal,  "decayNMomVecCal/D");
  ...
  tree->Branch("decayProtonVec",   event.decayProtonVec,  "decayProtonVec[3]/D");
  tree->Branch("decayProtonPos",   event.decayProtonPos,  "decayProtonPos[3]/D");
  ...
  tree->Branch("chisqrDecayProtonXY",   &event.chisqrDecayProtonXY,  "chisqrDecayProtonXY/D");
  tree->Branch("chisqrDecayProtonZ",   &event.chisqrDecayProtonZ,  "chisqrDecayProtonZ/D");
  tree->Branch("decayProtonMomCal",   &event.decayProtonMomCal,  "decayProtonMomCal/D");
  tree->Branch("decayProtonEkinCal",   &event.decayProtonEkinCal,  "decayProtonEkinCal/D");
  ...
  sprintf(buf1, "fClEdepForProtonDecay[%d]/D", FiberLayerNum+1);
  tree->Branch("fClEdepForProtonDecay", event.fClEdepForProtonDecay, buf1);
  sprintf(buf1, "fClEdepNormForProtonDecay[%d]/D", FiberLayerNum+1);
  tree->Branch("fClEdepNormForProtonDecay", event.fClEdepNormForProtonDecay, buf1);
  sprintf(buf1, "fClPathLengthForProtonDecay[%d]/D", FiberLayerNum+1);
  tree->Branch("fClPathLengthForProtonDecay", event.fClPathLengthForProtonDecay, buf1);
  ...
  tree->Branch("cClEdepForProtonDecay", &event.cClEdepForProtonDecay, "cClEdepForProtonDecay/D");
  ...
  tree->Branch("cRcEdepForProtonDecay", &event.cRcEdepForProtonDecay, "cRcEdepForProtonDecay/D");
}
