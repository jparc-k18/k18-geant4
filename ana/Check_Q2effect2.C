...
Check_Q2effect2( void )
{
  ...
  while( reader.Next() ){
	...
	std::cout<<"VP2x:"<<VP2x[0]
		 ...
		 <<", "<<VP2x[4]<<std::endl;
	for(int i=0; i<4; ++i){
	  hVP2x[i]->Fill(VP2x[0],VP2y[0]-VP2y[i+1]);
	  hVP2y[i]->Fill(VP2y[0],VP2x[0]-VP2x[i+1]);
	}
      }
	...
	std::cout<<"VP4x:"<<VP4x[0]
		 ...
		 <<", "<<VP4x[4]<<std::endl;
	for(int i=0; i<4; ++i){
	  hVP4x[i]->Fill(VP4x[0],VP4y[0]-VP4y[i+1]);
	  hVP4y[i]->Fill(VP4y[0],VP4x[0]-VP4x[i+1]);
	}
      }
    }
  }
