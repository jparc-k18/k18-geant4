...
FiberHit *FiberTrack::GetHit( std::size_t nth ) const
{
...
FiberHit *FiberTrack::GetHitOfLayerNumber( int lnum ) const
{
...
bool FiberTrack::DoFit( void )
{
  const std::string funcname = "[FiberTrack::DoFit()]";

  const DCConfMan & confMan=DCConfMan::GetInstance();

  std::size_t n = hitArray.size();
  if( n < FiberMinNHits ) return status_ = false;
