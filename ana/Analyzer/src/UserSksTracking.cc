...
bool ProcessOneEvent( std::ifstream &In )
{
  ...
  std::cout<< "*********************************************" <<std::endl;
#endif
    if( lnum<62 && lnumold<29 ){
      DCHit *hit = new DCHit( lnumold, 0, 0 );
      if( hit ){
	//	std::cout<<"pos1="<<pos<<std::endl;
	pos += MathTools::RandGauss(0., DCConfMan::GetInstance().GetResolution(lnumold));
  ...
  std::cout<<"nhin="<<nhin<<" nhout="<<nhout<<std::endl;
//   std::cout<<"nhinX0="<<trIn.GetX0()<<" nhoutX0="<<trOut.GetX0()<<std::endl;
//   std::cout<<"nhinY0="<<trIn.GetY0()<<" nhoutY0="<<trOut.GetY0()<<std::endl;
//   std::cout<<"nhinU0="<<trIn.GetU0()<<" nhoutU0="<<trOut.GetU0()<<std::endl;
//   std::cout<<"nhinV0="<<trIn.GetV0()<<" nhoutV0="<<trOut.GetV0()<<std::endl;
...
//   }
  ...
  // for (pip,pip)
  LorentzVector LvScat( sMom, sqrt(ProtonMass*ProtonMass+sMom.mag2()) );
