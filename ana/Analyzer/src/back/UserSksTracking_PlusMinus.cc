...
struct Event{
  ...
  double DC5Posl[MaxHits];
  double DC5Resd[MaxHits];
...
//   LorentzVector LvCore( 0., 0., 0., 0.0 );
  ...
  for( int i=0; i<nhdc; ++i ){
	...
	nhit4++;      
      ...
      if( layer==21 ){
	event.DC5Posl[nhit5] = posl;
	event.DC5Resd[nhit5] = resd;
	nhit5++;      
      }
    }
  }//std::cout<<"********************"<<std::endl;
...
void InitializeEvent( void )
{
  ...
  for( int it=0; it<MaxHits; it++){
    ...
    event.DC5Posl[it] = -999.0; 
    event.DC5Resd[it] = -999.0;
  }
...
void DefineHistograms( const char *filename )
{
  ...
  tree->Branch("DC5Hits", &event.DC5Hits, "DC5Hits/I");
  ...
  tree->Branch("DC5Posl", event.DC5Posl,"DC5Posl[DC5Hits]/D");
  tree->Branch("DC5Resd", event.DC5Resd,"DC5Resd[DC5Hits]/D");
